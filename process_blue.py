...
def process_blue_timeline(df, start_time):
    ...
    new_df = pd.DataFrame(data={'Label':df['Label'], 'Asset':df['To'], 'Color':'White', 'Hatch':'', 'Start':start_time['Vul Start'], 'End':start_time['Vul Start']}, index=[0])
    ...
    for index, row in df.iterrows():
        ...
        x = pd.DataFrame(data=[
            {'Label': label, 'Asset':asset, 'Color':'White', 'Hatch':'/', 'Start':nom_time,'End':pass_time},
            {'Label': label, 'Asset':asset, 'Color':'Yellow', 'Hatch':'', 'Start':pass_time, 'End':first_time},
            {'Label': label, 'Asset':asset, 'Color':'Green', 'Hatch':'', 'Start':first_time, 'End':last_time}
...
def process_jtac_timeline(df, start_time):
    ...
    #set first line of dataframe to be vul start with empty 'x' as placeholder
    new_df = pd.DataFrame(data={'Label':df['Label'], 'Asset':df['From'], 'Color':'White', 'Hatch':'', 'Start':start_time['Vul Start'], 'End':start_time['Vul Start']}, index=[0])
    ...
    for index, row in df.iterrows():
        ...
        last_time = row['Last Effect']
        x = pd.DataFrame(data=[
            {'Label': label, 'Asset':sender, 'Color':'White', 'Hatch':'/', 'Start':nom_time,'End':pass_time},
            {'Label': label, 'Asset':sender, 'Color':'Yellow', 'Hatch':'', 'Start':pass_time, 'End':first_time}
            ], index=[index, index+1])
        new_df = pd.concat([new_df, x])

    #Append Knock It Off to end of dataframe
    y = pd.DataFrame(data={'Label':df['Label'], 'Asset':df['From'], 'Color':'White', 'Hatch':'', 'Start':start_time['Knock It Off'], 'End':start_time['Knock It Off']}, index=[0])
    new_df = pd.concat([new_df, y])
